...
class Generator {
    ...
    save() {
        for (const enumTypeName of Object.keys(this.enumTypes)) {
            this.blocks.push(partials["enum-type"](this.enumTypes[enumTypeName]));
        }

        for (const objectTypeName of Object.keys(this.objectTypes)) {
            this.blocks.push(partials["object-type"](this.objectTypes[objectTypeName], this.getMobXType));
        }
        ...
        fs.writeFileSync(resolve(__dirname + "/../", this.outputPathTs),
            partials["imports-ts"](this.blocksTs), {
                encoding: "utf-8",
                flag: "w",
            });
    }
    ...
    getTypeInfo(type) {
        const rec = (type, modifiers = []) => {
            if (type.kind === "NamedType") {
                ...
                return rec(type.type, [...modifiers, type.kind])
            }
        };

        const typeInfo = rec(type);

        typeInfo.modifiers = [...typeInfo.modifiers, "NamedType"].reverse().reduce((acc, modifier) => {
            if (modifier === "NonNullType") {
                acc[acc.length - 1].notNull = true;
            } else {
                acc.push({
                    name: modifier,
                    notNull: false,
    ...
    getMobXType(typeInfo) {
        ...
        typeInfo.modifiers.forEach((pair) => {
            switch (pair.name) {
                case "ListType":
    ...
    buildQueryTypes(typeInfo) {
        this.blocksTs.push(...partials["query-type"](
            typeInfo,
